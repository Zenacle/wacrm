...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  return (
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-3 xl:grid-cols-6 shadow-sm">
        <Metric
          icon={<BarChart3 className="h-4 w-4 text-[#03B3C3]" />}
          label="Total Deals"
          value={String(stats.totalCount)}
          tooltip="Count of every deal in this pipeline that isn't marked as Lost. Won deals are still included."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-[#005662] dark:text-[#03B3C3]" />}
          label="Pipeline Value"
          value={formatCurrency(stats.totalValue)}
          tooltip="Sum of the dollar values of all deals in this pipeline, excluding deals marked as Lost."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-[#03B3C3]" />}
          label="Avg Deal Size"
        ...
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-[#005662] dark:text-[#03B3C3]" />}
        ...
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-[#03B3C3]" />}
          label="Won This Month"
          value={String(stats.wonThisMonth)}
          tooltip="Deals marked as Won since the first day of the current month."
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-500/80" />}
