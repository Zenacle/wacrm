...
function StageColumn({
  ...
  return (
    ...
    <div className="flex w-[85vw] min-w-[260px] max-w-[320px] shrink-0 snap-start flex-col rounded-xl border border-border bg-card p-4 shadow-sm lg:w-auto lg:max-w-none lg:flex-1 lg:basis-[260px] lg:shrink lg:snap-none">
      ...
      <div
        ...
        className={`mt-3 flex flex-1 flex-col gap-2 rounded-lg transition-all ${
          isOver
            ? "bg-[#03B3C3]/5 outline outline-2 outline-dashed outline-[#03B3C3] outline-offset-2"
            : ""
        }`}
      >
        {deals.length === 0 ? (
          <div className="flex flex-1 items-center justify-center rounded-lg border-2 border-dashed border-border py-10 text-xs text-muted-foreground bg-muted/10">
            Drop a deal here
          </div>
        ) : (
          deals.map((deal) => (
      ...
      <Button
        ...
        className="mt-3 w-full justify-start border border-dashed border-border bg-transparent text-muted-foreground hover:bg-muted hover:text-foreground"
      >
