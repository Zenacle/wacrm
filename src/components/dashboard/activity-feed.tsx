...
const KIND_THEME: Record<ActivityKind, KindTheme> = {
  message: { icon: MessageSquare, badge: 'bg-brand-cyan/10 text-brand-cyan' },
  contact: { icon: UserPlus, badge: 'bg-brand-cyan/10 text-brand-cyan' },
  deal: { icon: Briefcase, badge: 'bg-brand-cyan/10 text-brand-cyan' },
  broadcast: { icon: Radio, badge: 'bg-brand-teal/10 text-brand-teal dark:bg-brand-cyan/10 dark:text-brand-cyan' },
  automation: { icon: Zap, badge: 'bg-brand-cyan/10 text-brand-cyan' },
}
...
export function ActivityFeed({ items, loading }: ActivityFeedProps) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card shadow-sm hover:shadow-md transition-all">
        ...
        <>
          <ul className="divide-y divide-border">
            {visible.map((it, i) => {
              ...
              const stripe = i % 2 === 0 ? 'bg-transparent' : 'bg-muted/10'
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm text-foreground/85 font-medium">
                    {it.text}
                  </span>
